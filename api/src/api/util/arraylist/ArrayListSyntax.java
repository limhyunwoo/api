 ...
 * java.util.ArrayList는 동기화가 되어 있지 않다.
 *  
 */
 ...
 * 원래 기본적으로 동기화되지만, ArrayList 는 그렇지 않다.
 * 2.비동기화 된 컬렉션은 동기화된 컬렉션보다 더 나은
 * 성능을 제공하며 스레드들이 컬렉션을 공유하지 않는 프로그램에서는
 * 일반적으로 ArrayList 를 Vector 보다 더 선호하며
 * ArrayList 는 동기화를 고려하지 않는 Vector 처럼
 * 동작함으로써 스레드 동기화에 따르는 부담을 가지지 않기 때문에 Vector 보다 빠르기 실행한다.
 * 
 */
